...
export default function FilterModal({ open = false, onClose = () => {}, onApply = () => {} }) {
  ...
  const positionsRef = useRef({});
  const [sortExpanded, setSortExpanded] = useState(false);
  const isScrollingToSection = useRef(false); // Track programmatic scrolling
  ...
  const scrollToSection = (key) => {
    ...
    }, 350); // slightly longer than scroll animation
  };
  ...
  // handle scroll event to update active section based on scroll position
  const handleScroll = (e) => {
    // Don't update active key during programmatic scrolling
    if (isScrollingToSection.current) return;

    const scrollY = e.nativeEvent.contentOffset.y;
    const positions = Object.entries(positionsRef.current).sort((a, b) => a[1] - b[1]);

    // Find which section is currently in view (with 100px threshold from top)
    let currentKey = positions[0]?.[0] || 'sort';
    for (let i = positions.length - 1; i >= 0; i--) {
      if (scrollY + 100 >= positions[i][1]) {
        currentKey = positions[i][0];
        break;
      }
    }
